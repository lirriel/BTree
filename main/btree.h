...
protected:
    ...
    IComparator* _comparator;
        ...
        /**
            * \brief Всспомогательный метод для поиска всех совпадений
            * @param pw Страница, с которой начинается поиск
            * @param k Искомое значение ключа
            * @param keys Список совпадений в массиве
            */
        void searchAll_help(PageWrapper& pw, const Byte *k, std::list<Byte*>& keys);

        /**
         * \brief Вспомогательный метод для рекурскивного поиска в дереве
         * @param pw Страницы, с которой начинается поиск
         * @param k Искомое значение ключа
         * @return Найденное значение ключа
         */
        Byte* search_help(PageWrapper& pw, const Byte *k);
}; // class BaseBTree
