...
Byte *BaseBTree::search_help(BaseBTree::PageWrapper &pw, const Byte *k)
{
    int i = 0;

    while (i < pw.getKeysNum() && getComparator()->compare(pw.getKey(i), k, getKeysSize()))
        i++;

    if (i <= pw.getKeysNum() - 1 && getComparator()->isEqual(pw.getKey(i), k, getKeysSize()))
        return getRootPage().getKey(i);
    else
        if (pw.isLeaf())
            return nullptr;

    PageWrapper pw1(this);
    pw1.readPageFromChild(pw, i);

    return search_help(pw1, k);;
}

void BaseBTree::searchAll_help(BaseBTree::PageWrapper &pw, const Byte *k, std::list<Byte *> &keys)
{
    int i = 0;
    ...
    PageWrapper pw1(this);
    while (i <= pw.getKeysNum() - 1 && getComparator()->isEqual(pw.getKey(i), k, getKeysSize()))
    {
        keys.push_back(getRootPage().getKey(i));

        if (!pw.isLeaf())
        {
            pw1.readPageFromChild(pw, i);
            searchAll_help(pw1, k, keys);
        }
...
{
    return search_help(getRootPage(), k);
}

int BaseBTree::searchAll(const Byte* k, std::list<Byte*>& keys)
{
    searchAll_help(getRootPage(), k, keys);
    return keys.size();
}
...
void BaseBTree::insert(const Byte *k) {
    ...
    if (_rootPage.getKeysNum() == getMaxKeys())
    {
        PageWrapper s(this);
        s.allocNewRootPage();
        s.setAsRoot(true);
        s.setLeaf(false);
        s.setCursor(0, getRootPage().getPageNum());
        s.splitChild(0);
        s.insertNonFull(k);
    }
    else
        getRootPage().insertNonFull(k);
}
...
{
    if (isFull())
        throw std::domain_error("A parent node is full, so its child can't_1 be splitted");

    if (iChild > getKeysNum())
        throw std::invalid_argument("Cursor not exists");

    PageWrapper y(_tree);
    y.readPageFromChild(*this, iChild);

    UShort t_1 = (UShort) _tree->getMinKeys();

    PageWrapper z(_tree);
    z.allocPage(t_1, y.isLeaf());

    z.setKeyNum(t_1);

    z.copyKeys(z.getKey(0), y.getKey(t_1 + 1), t_1 + 1);

    if (!y.isLeaf())
        z.copyCursors(z.getCursorPtr(0), y.getCursorPtr(t_1 + 1), t_1 + 1);


    setKeyNum(getKeysNum() + 1);

    for (UInt i = getKeysNum() - 1; i > iChild; --i)
        setCursor(i + 1, getCursor(i));

    setCursor(iChild + 1, z._pageNum);

    for (int i = getKeysNum() - 2; i >= iChild; --i)
        copyKey(getKey(i + 1), getKey(i));

    Byte* b = y.getKey(t_1);

    copyKey(getKey(iChild), y.getKey(t_1));

    y.setKeyNum(t_1);
    z.setKeyNum(t_1);
    setKeyNum(getKeysNum());
...
void BaseBTree::PageWrapper::insertNonFull(const Byte* k)
{
    ...
    if (isLeaf())
    {
        ...
        while (i >= 0 &&  _tree->getComparator()->compare(k, getKey(i), _tree->getKeysSize()))
        {
            copyKey(getKey(i + 1), getKey(i));
            i--;
        }
    ...
    else
    {
        ...
        PageWrapper w(_tree);
        w.readPageFromChild(*this, i);

        if (w.getKeysNum() == _tree->getMaxKeys())
        {
            splitChild(i);

            if (_tree->getComparator()->compare(getKey(i), k, _tree->getKeysSize()))
                i++;
        }
